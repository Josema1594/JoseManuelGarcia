    ...
    <section id="cv" className="py-16 px-4 sm:py-20 sm:px-6 lg:py-24 lg:px-8 bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="max-w-5xl mx-auto">
        <h2 className="text-3xl sm:text-4xl lg:text-4xl font-bold text-center mb-12 lg:mb-16 text-slate-800">
          {t('cv.section.heading', { defaultValue: 'Curriculum Vitae' })}
        </h2>
        
        <p className="text-base sm:text-lg lg:text-xl text-slate-600 mb-12 sm:mb-16 leading-relaxed max-w-3xl mx-auto text-center">
          {t('cv.section.intro', { defaultValue: 'Descarga mi CV completo para conocer más detalles sobre mi experiencia, educación y habilidades técnicas.' })}
        </p>
          ...
          <div className="mb-8">
            ...
            <div className="text-center">
              <p className="text-sm sm:text-base text-slate-500 bg-slate-50 px-3 py-1 rounded-full inline-block">
                {t('cv.section.lastUpdated', { defaultValue: 'Última actualización: Agosto 2025' })}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 lg:gap-8 text-center">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-lg border border-slate-200 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            ...
            <h4 className="text-lg sm:text-xl font-bold text-slate-800 mb-3">{t('cv.section.stats.experience.title', { defaultValue: 'Experiencia' })}</h4>
            <p className="text-sm sm:text-base text-slate-600 leading-relaxed">{t('cv.section.stats.experience.value', { defaultValue: '2 años en desarrollo web' })}</p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-lg border border-slate-200 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            ...
            <h4 className="text-lg sm:text-xl font-bold text-slate-800 mb-3">{t('cv.section.stats.education.title', { defaultValue: 'Educación' })}</h4>
            <p className="text-sm sm:text-base text-slate-600 leading-relaxed">{t('cv.section.stats.education.value', { defaultValue: 'Desarrollador de Aplicaciones Web' })}</p>
          </div>
          
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-lg border border-slate-200 hover:shadow-xl transition-all duración-300 transform hover:-translate-y-1">
            ...
            <h4 className="text-lg sm:text-xl font-bold text-slate-800 mb-3">{t('cv.section.stats.skills.title', { defaultValue: 'Habilidades' })}</h4>
            <p className="text-sm sm:text-base text-slate-600 leading-relaxed">{t('cv.section.stats.skills.value', { defaultValue: 'Frontend & Backend' })}</p>
          </div>
        </div>
        ...
        <div className="mt-16 sm:mt-20 pt-8 border-t border-slate-200">
          <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-slate-200 text-center">
            <p className="text-sm sm:text-base text-slate-600 font-medium">
              {t('cv.footer.rights', { defaultValue: '© 2025 José Manuel García Muñoz. Todos los derechos reservados.' })}
            </p>
            <p className="text-xs sm:text-sm text-slate-500 mt-2">
              {t('cv.footer.builtWith', { defaultValue: 'Portfolio desarrollado con React, Vite y Tailwind CSS' })}
            </p>
          </div>
        </div>
      </div>
